...
public:
    ...
    SuffixNode* getCurrNode() {
        return active->activeNode->child[int(input[active->activeEdge])];
    }
    ...
    void walkDown(int idx) {
        ...
        if(diff(node) < active->activeLength){
            ...
            active->activeEdge = node->child[int(input[idx])]->start;
        }
        else{
    ...
    void startPhase(int idx) {
        ...
        while(remaining > 0) {
            if(active->activeLength == 0) {
                ...
                if(active->activeNode->child[int(input[idx])]) {
                    currNode = active->activeNode->child[int(input[idx])];
                }
                ...
                if(currNode) {
                    ...
                    root->child[int(input[idx])] = new SuffixNode(idx, end);
                    remaining -= 1;
                }
            }
            else {
                ...
                if(ch != EOP_CHAR) {
                    if(ch == input[idx]) {
                        ...
                        newInternalNode->child[int(input[newInternalNode->start + active->activeLength])] = node;
                        newInternalNode->child[int(input[idx])] = newLeafNode;
                        newInternalNode->index = -1;
                        active->activeNode->child[int(input[newInternalNode->start])] = newInternalNode;
                    ...
                    node->child[int(input[idx])] = new SuffixNode(idx, end);
    ...
    */
    ...
    bool validateSuffixTreeHelper(SuffixNode* root, int idx, int curr) {
        ...
        SuffixNode* node = root->child[int(input[curr])];
